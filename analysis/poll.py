...
class Document:
	def __init__(self, title, tables, currDir, transposedDir):
		...
		strings.append('<link rel="stylesheet" href="http://netdna.bootstrapcdn.com/bootstrap/3.0.3/css/bootstrap.css">')
...
class CrossTab:
	...
	def write(self, id):
		...
		strings.append('<script src="http://netdna.bootstrapcdn.com/bootstrap/3.0.3/js/bootstrap.js"></script>')
...
class PollData:
	...
	def parseQuestions(self, questionsFile):
		questions = {}
		with open(questionsFile, 'rU') as csvFile:
	...
	def parseEntry(self, row, question):
		...
		answers = [option for option in question.listOptions if option.strip() in entry]
		...
		if (len(answers2) < 1):
			if entry != "":
				print '---'
				print entry
				for option in question.listOptions:
					print option
					print (option in entry)
				print '---'
			return None
		return answers2
